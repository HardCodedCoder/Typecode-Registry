...
The high-level architecture provides a comprehensive overview of the main components and their interactions within the system, as depicted in Figure \ref{fig:system_overview}. The architecture comprises a frontend, a backend service, a database, and the Microsoft OAuth 2.0 authentication service.
...
\begin{itemize}
    ...
    \item \textbf{Logging:} For application logging, we employ Golang's standard `log` package.
    This facilitates a straightforward and effective logging mechanism, essential for debugging, monitoring, and auditing backend operations.
    The `log` package's simplicity and integration with Go's ecosystem make it an ideal choice for our application's logging needs.
\end{itemize}
...
\begin{table}[H]
    \begin{tabularx}{\textwidth}{|l|l|X|}
        \hline
        \rowcolor{headercolor} \textbf{Route} & \textbf{HTTP Methods} & \textbf{Description} \\
        \hline
        /healthcheck & GET & This route returns the current state of the API and can be used to retrieve status information. \\
        \hline
        /items & GET, POST & This route creates a new item.
        In the request body, the client needs to send the following data:
        ...
        \hline
        /items/ & GET, PUT, & \multirow{Gets, updates, or deletes an item by its ID.} \\
        & DELETE & \\
        \hline
...
\headingFour{Logging}
Utilizing Go's standard `log` package, our backend implements a comprehensive logging system.
It's configured to capture both system-level and application-level events, aiding in debugging and monitoring the application's health.

This module breakdown provides an overview of our backend's architecture, illustrating the cohesive design and the strategic choice of technologies that ensure our application's reliability and performance.
...
\headingFour{Key Components and Features}
